  ...
  <div>
    ...
    <section class="body">
      <div class="section_top container">
        <router-link :to="`/fundraisers/${slug}`">Return to Fundraiser Page</router-link>
      </div>
      <div class="section_body" id="donation">
        <div class="body_top">
          <h3>{{ `Donate to Fundraiser ${organization.name}` }}</h3>
        </div>
        <div class="body_main">
          <div class="body_left">
...
export default {
  ...
  async created() {
    ...
    await this.apiGetfundraiserData({ slug: this.slug })
  ...
  methods: {
    ...
    ...mapActions('fundraisers', ['apiGetfundraiserData', 'apiGetNumberShared']),

    commissionCalculation() {
      const cardType = this.cardTypeSelect
      const price = +this.price
      const paymentType = 'default'
      let result = 0

      if (this.payСommission) {
        result = commissionCalculation(price, cardType, paymentType)
      }
