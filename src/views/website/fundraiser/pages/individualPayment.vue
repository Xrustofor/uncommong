  ...
  <div>
    ...
    <section class="body">
      <div class="section_top container">
        <router-link :to="`/fundraisers/${slug}/individuals/${individualID}`"
          >Return to Individual Page</router-link
        >
      </div>
      <div class="section_body" id="donation">
        <div class="body_top">
          <h3>{{ `Donate to Individual ${fundraiser.organization.name}` }}</h3>
        </div>
        <div class="body_main">
          <div class="body_left">
...
export default {
  ...
  async created() {
    ...
    const { fundraiserSlug, individualID } = this.$route.params
    const data = {
      slug: fundraiserSlug,
      organizationFundraiserIndividual: +individualID,
    }

    await this.apiGetFundraiserIndividual(data)
  ...
  computed: {
    ...mapGetters('fundraisers/individual', {
  ...
  methods: {
    ...mapActions('individual', ['apiGetNumberShared']),
    ...mapActions('fundraisers/individual', ['apiGetFundraiserIndividual']),
    ...
    sharedAction() {
      this.apiGetNumberShared({
        ...
        individualId: this.individual.i,
      })
    },
