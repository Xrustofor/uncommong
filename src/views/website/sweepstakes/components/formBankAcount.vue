...
export default {
  ...
  watch: {
    idOrganizationSelect(val) {
      if (val) {
        this.bankAccountsOptionsn = this.bankAccountsOptionsn.map((b) => {
          return b.disabled ? { ...b, disabled: false } : b
        })
        this.organizationSelect = this.organizations.find((o) => o.id === val)
      } else {
        ;(this.bankAccountSelect = null),
          (this.bankAccountsOptionsn = this.bankAccountsOptionsn.map((b) => {
            return b.disabled === false ? { ...b, disabled: true } : b
          }))
      }
    },
  ...
  methods: {
    ...mapActions('publicweb/general', ['apiDonationBankAccountSweepstake']),

    async send() {
      const result = {
        organizationId: this.idOrganizationSelect,
      ...
      const item = await this.apiDonationBankAccountSweepstake(result)
